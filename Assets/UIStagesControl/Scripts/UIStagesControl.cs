...
namespace ERA
{
    public class UIStagesControl : MonoBehaviour
    {
        ...
        [HideInInspector] [SerializeField] private bool _isDisableAll;
        ...
        private bool[] _isDisabledOriginStages;
            ...
            foreach (Transform stage in stages.transform)
            {
                if (stage.TryGetComponent(out UIStageButton uiStageButton))
                {
                    ...
                    uiStageButton.AddActionOnClick = () =>
                    {
                        ...
                        restartButton.SetActiveStage = _activeStage;
                        DisableAllStage();
        ...
        private void SwitchStagesWithKeyboard()
        {
            ...
            foreach (var stageButton in _stageButtons)
            {
                ...
                {
                    if (!uiStageButton.IsActive)
                    {
                        uiStageButton.OnClickInvoke();
                        break;
                    }
        ...
        private void SwitchIsDisableAll(bool isDisableAll)
        {
            if (_stageButtons == null || _isDisabledOriginStages == null) return;

            for (var i = 0; i < _stageButtons?.Count; i++)
            {
                _stageButtons[i].IsDisabled = isDisableAll || _isDisabledOriginStages[i];
            }
        }

        private void RefreshIsDisabledOriginStages()
        {
            _isDisabledOriginStages ??= new bool[_stageButtons.Count];
            for (var i = 0; i < _stageButtons?.Count; i++)
            {
                _isDisabledOriginStages[i] = _stageButtons[i].IsDisabled;
            }
        }
...
//-----CUSTOM EDITOR-----
    ...
    public class UIStagesControlEditor : Editor
    {
        ...
        private void DrawButtonNameLab()
        {
            ...
            var text = new SerializedObject(_propertyTextNameLab.objectReferenceValue).FindProperty("m_text")
                .stringValue;
        ...
        private void DrawButtonMinusStage()
        {
            ...
            GUI.backgroundColor = Color.white;
            if (GUILayout.Button("-",
                    new GUIStyle(GUI.skin.button) { fontSize = 30, padding = new RectOffset(0, 0, 0, 5) },
                    _sizeStageButton))
            {
        ...
        private void ShowOptionTextStageAndSteps()
        {
            ...
            if (_isFoldoutOptionsText)
            {
                EditorGUILayout.HelpBox(
                    "Entering data manually is not recommended. Use the API to write data from a file!",
                    MessageType.Warning);
                EditorGUI.indentLevel++;
